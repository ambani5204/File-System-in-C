...
//it will assign 3 blocks to store the data if it empty then 00 
//otherwise it will give the adress of data block where all things are stored of file or directories
typedef struct
{
...
int INB;						// total number of entries in inode table here 128
...
/* 
	here itos function line 2
		sprintf(st, "%0*d", n, num); it is convert the num to st string in provided format and 
	is indicate that answer is stored in the st array and 
	%0*d is a format specifier in which 
		%d is for integer to be formatted
		* indicate that the width of formatted output is specified by an argument here it will n here 
			means size of the string
		0 indicate that number should be padded with leading zeroes if it is shorter than provided width


		strncpy(s, st, n);
		this function will copy the resultant string into s 
*/
...
/*
	SFS::%s# " is a format string that specifies how the output should be formatted.
	SFS:: is a literal string that will be printed as-is.
	%s is a placeholder for a string that will be provided as an argument to printf.
	# is another literal string that will be printed as-is.
*/
void printPrompt()
{
